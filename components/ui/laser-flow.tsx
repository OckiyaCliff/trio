...
export function LaserFlow({
    ...
    useEffect(() => {
        ...
        const renderer = new THREE.WebGLRenderer({ antialias: false, alpha: false, depth: false, stencil: false, powerPreference: 'high-performance', premultipliedAlpha: false, preserveDrawingBuffer: false, failIfMajorPerformanceCaveat: false, logarithmicDepthBuffer: false })
        ...
        const material = new THREE.RawShaderMaterial({ vertexShader: VERT, fragmentShader: FRAG, uniforms, transparent: false, depthTest: false, depthWrite: false, blending: THREE.NormalBlending })
        const mesh = new THREE.Mesh(geometry, material)
        mesh.frustumCulled = false
        scene.add(mesh)

        const clock = new THREE.Clock()
        let prevTime = 0, fade = hasFadedRef.current ? 1 : 0
        const mouseTarget = new THREE.Vector2(0, 0), mouseSmooth = new THREE.Vector2(0, 0)
        ...
        setSizeNow()
        const ro = new ResizeObserver(scheduleResize); ro.observe(mount)
        const io = new IntersectionObserver(entries => { inViewRef.current = entries[0]?.isIntersecting ?? true }, { root: null, threshold: 0 }); io.observe(mount)
        const onVis = () => { pausedRef.current = document.hidden }
        document.addEventListener('visibilitychange', onVis, { passive: true })

        const updateMouse = (clientX: number, clientY: number) => {
            const rect = rectRef.current; if (!rect) return
            const x = clientX - rect.left, y = clientY - rect.top, ratio = currentDprRef.current
        ...
        const animate = () => {
            ...
            const t = clock.getElapsedTime(), dt = Math.max(0, t - prevTime)
            prevTime = t
            const dtMs = dt * 1000
            ...
            if (!hasFadedRef.current) { fade = Math.min(1, fade + cdt); uniforms.uFade.value = fade; if (fade >= 1) hasFadedRef.current = true }
            const tau = Math.max(1e-3, mouseSmoothTimeRef.current), alpha = 1 - Math.exp(-cdt / tau)
            ...
            if (now - lastFpsCheckRef.current > 750 && fpsSamplesRef.current.length > 0) {
                ...
                if (avg < 50) next = clamp(currentDprRef.current * 0.85, 0.6, baseDprRef.current)
                else if (avg > 58 && currentDprRef.current < baseDprRef.current) next = clamp(currentDprRef.current * 1.1, 0.6, baseDprRef.current)
                if (Math.abs(next - currentDprRef.current) > 0.01) { currentDprRef.current = next; setSizeNow() }
        ...
        return () => {
            cancelAnimationFrame(raf); if (resizeRaf) cancelAnimationFrame(resizeRaf)
            ro.disconnect(); io.disconnect()
            document.removeEventListener('visibilitychange', onVis)
            canvas.removeEventListener('pointermove', onMove as any)
            canvas.removeEventListener('pointerleave', onLeave as any)
            scene.clear(); geometry.dispose(); material.dispose(); renderer.dispose(); renderer.forceContextLoss()
    ...
    useEffect(() => {
        ...
        u.uFlowSpeed.value = flowSpeed; u.uVLenFactor.value = verticalSizing; u.uHLenFactor.value = horizontalSizing
        u.uFogIntensity.value = fogIntensity; u.uFogScale.value = fogScale; u.uWSpeed.value = wispSpeed
        u.uWIntensity.value = wispIntensity; u.uFlowStrength.value = flowStrength
        u.uDecay.value = decay; u.uFalloffStart.value = falloffStart; u.uFogFallSpeed.value = fogFallSpeed
