...
function DockItem({ children, className = '', onClick, mouseX, spring, distance, magnification, baseItemSize }: DockItemProps) {
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`relative inline-flex items-center justify-center rounded-full bg-card border border-border ${className}`}
            tabIndex={0}
            role="button"
        >
    ...
    return (
        <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
            <motion.div
                ...
                className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-4 rounded-2xl border border-border bg-card/80 backdrop-blur-md pb-2 px-4`}
